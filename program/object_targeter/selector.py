...
class ObjectSelector(IObjectSelector):
    ...
    def __init__(self, max_target_wait = 5):
        self.__target = None
        self.__max_target_wait = max_target_wait
    ...
    def select(self, results: list[Results], mode: str = 'first',
        ...
        if target is not None:
            ...
            if box is not None:
                ...
                self.__waited = 0
                return (x1 + x2) // 2, (y1 + y2) // 2
            elif self.__waited < self.__max_target_wait:
                self.__waited += 1
                return  None
        else:    
            self.__waited = 0
